...
To Do: Add lines for model predictions 
...
est_summary <- wl2_establishment %>% 
  group_by(pop, elev_m, Geographic_Dist, GD_Recent_Wtr_Year_2023, GD_Historic_Wtr_Year_2023, 
           ...
           TempDist_Recent_GrwSsn_2023, GD_Historic_GrwSsn_2023) %>% 
...
wl2_tot_fecundity_summary <- wl2_tot_fecundity %>% 
  ...
  group_by(pop, elev_m, AvgTempDist_Recent_Wtr_Year, 
...
fig2_sub <- ggarrange(WYtemp_probfit, WYtemp_totfruit,
                        labels = c("A)", "B)"), 
...
figS5_sub <- ggarrange(WYtemp_est, WYtemp_y1surv, 
                        WYppt_est, 
                        labels = c("A)", "B)", "C)"), 
                        font.label = list(size=30, face = "plain"), 
                        ncol=2, nrow=2) 
...
## Figure S6: Growth Season Clim Dist - Temp and PPT
```{r}
...
figS6_sub <- ggarrange(est_temp, NULL,
                         totrep_temp, totrep_ppt,
                        labels = c("A)", "", "B)"), 
                        font.label = list(size=30, face = "plain"), 
                        ncol=2, nrow=2) 
ggsave("../Figures/FigureS6_GrwSsn.png", width = 26, height = 18, units = "in")
```

## Figure S7: Additional Gower's recent water year climate distance 
```{r}
...
figS7_gower <- ggarrange(est_gower, y1surv_gower, 
                        wintersurv_gower, tot_fit_gower,
                        labels = c("A)", "B)", "C)", "D)"), 
                        font.label = list(size=30, face = "plain"), 
                        ncol=2, nrow=2) 
ggsave("../Figures/FigureS7_WTRYRGowers.png", width = 26, height = 18, units = "in")
```
...
est_gowers <- est_summary %>% 
  ...
  geom_point(size=6) + 
  geom_errorbar(aes(ymin=meanEst-semEst,ymax=meanEst+semEst),width=.02, linewidth = 2) +
  annotate("text", x = 0.23, y= 0.5714286, label = "WL2", 
           colour = "purple", fontface="bold", size = 22 / .pt) +
  theme_classic() + 
  scale_y_continuous(expand = c(0.01, 0)) +
...
repsurvy2_gowers <- wl2_surv_to_bud_y2_summary %>% 
  ggplot(aes(x=GD_Recent_GrwSsn_2024, y=meanSurvtoBudY2, group = pop, colour = elev_m)) +
  scale_colour_gradient(low = "#F5A540", high = "#0043F0") +
  geom_point(size=6) + 
  geom_errorbar(aes(ymin=meanSurvtoBudY2-semEst,ymax=meanSurvtoBudY2+semEst),width=.02,linewidth = 2) +
  annotate("text", x = 0.23, y= 0.8333333, label = "WL2", 
           colour = "purple", fontface="bold", size = 22 / .pt) +
  theme_classic() + 
  scale_y_continuous(expand = c(0.01, 0), limits = c(0.4,1.05)) +
...
figS8_gower <- ggarrange(est_gowers, y1_surv_gowers, 
                        repsurvy2_gowers, totrep_gowers,
                        labels = c("A)", "B)", "C)", "D)"), 
                        font.label = list(size=30, face = "plain"), 
                        ncol=2, nrow=2) 
...
figS9_geo <- ggarrange(probfit_geo, est_geo, repsurvy2_geo, 
                        labels = c("A)", "B)", "C)"), 
                        font.label = list(size=30, face = "plain"), 
                        ncol=2, nrow=2) 
