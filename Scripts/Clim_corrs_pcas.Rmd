...
# Code for Figures 1, 3, S2, S3; Table S2 and S3 
...
ggplot() +
  ...
  theme_void()
#ggsave("../Figures/Pop_Map_Figure1b.png", width = 10, height = 10, units = "in")
```

## Climate variable correlations (Figure S2)
...
## Climate Change PCAs (Figure 3; Table S2, S3)
...
```{r}
season_facet_labels <- as_labeller(c('Growth Season' = 'A) Growth Season', 'Water Year' = 'B) Water Year'))
...
home_sites_pca_allclim <- all_clim_avgs_locs.pc %>%  
  ...
  facet_wrap(~Season, labeller = season_facet_labels)
...
#### Water Year Only
```{r}
home_sites_pca_allclim_wtryr <- all_clim_avgs_locs.pc %>%  
...
## add WL2 garden 2023 and 2024
home_sites_pca_allclim_wtryr$data <- rbind(home_sites_pca_allclim_wtryr$data, 
  data.frame(
    parent.pop = "WL2_Garden",
    elev_m = 2020,
    Lat = NA,
...
WL2_label_wtryr <- data.frame(
  label = c("WL2 \n 2023", "WL2 \n 2024"),
  Season   = c("Water Year", "Water Year"),
  x     = c(-4.5, -1.5),
  y     = c(6.5, 2.5),
  timeframe = NA
)

home_sites_pca_allclim_wtryr + 
  geom_point(data=filter(home_sites_pca_allclim_wtryr$data, parent.pop == "WL2_Garden", Season=="Water Year"), 
             size=6, shape = 18, show.legend = FALSE) +
  geom_text(data = WL2_label_wtryr,
  mapping = aes(x = x, y = y, label = label),
  colour = "#8959c9",
  size=7
) +
  geom_path(data=filter(home_sites_pca_allclim_wtryr$data, parent.pop == "WL2"),
            aes(group=group),arrow = arrow(length=unit(5, "points"), 
                                           type="closed"), linewidth = 1.9) 
ggsave("../Figures/AllClim_PC1-PC2_PlusGarden_WtrYr.png", width = 8, height = 6, units = "in")
```

## All climate PCA (all years, water year, growing season, historic and recent) - Fig 1a, S3
...
autoplot(allclim.pc, data = all_clim,
         ...
         loadings.label.vjust = 1.3, loadings.label.repel=TRUE) +
   scale_colour_gradient(low = "#F5A540", high = "#0043F0") +
  labs(color="Elevation (m)") +
  geom_vline(xintercept = 0, linetype="dashed") + geom_hline(yintercept = 0, linetype="dashed") +
  theme_classic() +
  theme(text=element_text(size=28))
ggsave("../Figures/AllClim_YrlyVar.png", width = 9.5, height = 6, units = "in")
