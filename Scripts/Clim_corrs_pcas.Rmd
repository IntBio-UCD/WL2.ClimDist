...
-   Add loadings labels to hulls PCA
...
home_sites_pca_allclim <- all_clim_avgs_locs.pc %>%  
  ...
  annotate("text", x = -5.8, y = -5, label = "Cold/Snow", size=6) +
  annotate("text", x = 3, y = -5, label = "Warm/No Snow", size=6) +
...
home_sites_pca_allclim + 
  ...
  geom_path(data=filter(home_sites_pca_allclim$data, parent.pop == "WL2"),
            aes(group=group),arrow = arrow(length=unit(5, "points"), type="closed"), linewidth = 1.9) 
