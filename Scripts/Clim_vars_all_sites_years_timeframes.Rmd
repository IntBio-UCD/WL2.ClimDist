...
pck_sum_60_pop_avgs <- flint_all_year_wtr_yr %>% #based on water year 
  filter(parent.pop!="UCD_Garden") %>% #remove Davis garden 
  filter(timeframe=="historic" | timeframe=="recent") %>% #past 60 years 
  group_by(parent.pop, elevation.group, elev_m, Lat, Long, wtr_yr) %>% 
  summarise(pck_sum = sum(pck, na.rm = TRUE)) %>% #sum for each water year
  ungroup() %>% 
  group_by(parent.pop, elevation.group, elev_m, Lat, Long) %>% 
  summarise(PckSum=mean(pck_sum, na.rm=TRUE)) #avg annual pck sum 

cwd_60_avgs <- pop_elev_climate_60 %>% 
  group_by(parent.pop, elevation.group, elev_m, Lat, Long, month) %>% 
  summarise(cwd_avg=mean(cwd, na.rm=TRUE)) #60 year averages per month for each pop

pop_elev_climate_60_sums_avgs <- left_join(cwd_60_avgs, pck_sum_60_pop_avgs)
```
...
```{r}
nosnow_pops_60 <- pop_elev_climate_60_sums_avgs %>% filter(PckSum < 70)
...
```{r}
snow_pops_60 <- pop_elev_climate_60_sums_avgs %>% filter(PckSum >= 70)
...
## Check if snowy 2023 --> shorter growing season
```{r}
snow_grwseason_60 %>% filter(parent.pop=="WL2_Garden") %>% filter(year_cal=="2023") %>%
  mutate(ssnlength=lastmonth-firstmonth)
snow_grwseason_60 %>% filter(parent.pop=="WL2_Garden") %>% select(parent.pop:year_cal) %>% 
  distinct() %>% 
  mutate(ssnlength=lastmonth-firstmonth) %>% 
  ungroup() %>% 
  summarise(meanLength=mean(ssnlength))
#average is 6.5, in 2023 it was 5
snow_grwseason_60 %>% filter(parent.pop=="WL2_Garden") %>% 
  select(parent.pop:year_cal) %>% 
  distinct() %>% 
  mutate(ssnlength=lastmonth-firstmonth) %>% 
  ungroup() %>% 
  filter(ssnlength <= 5) #season length has been 5 months 13 times in the last 60 years --> natural occurence
```
