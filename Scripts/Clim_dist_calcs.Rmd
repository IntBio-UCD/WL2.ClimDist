...
-   Fix ordering of populations on x axis of figures 
...
```{r}
all_clim_scaled <- all_clim %>% 
  filter(parent.pop!="WL2_Garden") %>% 
  mutate_at(c("ann_tmean","ann_ppt","cwd","pck", "elev_m"),scale)
temp_mod <- lmer(ann_tmean ~ elev_m*timeframe*Season + (1|parent.pop), data=all_clim_scaled)
summary(temp_mod)
ppt_mod <- lmer(ann_ppt ~ elev_m*timeframe*Season + (1|parent.pop), data=all_clim_scaled)
summary(ppt_mod)
pck_mod <- lmer(pck ~ elev_m*timeframe*Season + (1|parent.pop), data=all_clim_scaled)
summary(pck_mod)
cwd_mod <- lmer(cwd ~ elev_m*timeframe*Season + (1|parent.pop), data=all_clim_scaled)
summary(cwd_mod)
```
