...
# Code for calculating climate distance (Figure S4)
...
library(corrplot) #test sig of correlations 
```
...
wl2_gowers_avg %>% 
  ...
  ggplot(aes(x=fct_reorder(parent.pop, elev_m), y=Avg_Gowers, group=parent.pop, fill=elev_m)) +
  geom_col(width = 0.7,position = position_dodge(0.75)) +
  scale_y_continuous(expand = c(0, 0)) +
  scale_fill_gradient(low = "#F5A540", high = "#0043F0") +
  labs(y="Gowers Envtal Distance \n from WL2", fill="Elevation (m)", x="Population") +
...
### Check how much warmer 2024 was compared to previous years and how that temp diff reflects the expected diff b/t low elev pops and the garden 
```{r}
#compare WL2 2024 temp to avg WL2 garden temp
all_clim %>% filter(parent.pop=="WL2_Garden", year==2024, Season=="Growth Season") #ann_tmean = 14.29417	
all_clim %>% filter(parent.pop=="WL2_Garden", year>1993, Season=="Growth Season") %>% 
  summarise(AvgTemp=mean(ann_tmean)) #avg = 11.83691		
#temp diff = 2.45726
all_clim %>% filter(parent.pop=="WL2_Garden", year>1963, Season=="Growth Season") %>% 
  summarise(AvgTemp=mean(ann_tmean)) #avg = 11.42444			
#temp diff = 2.86973

#calculate average temp diff b/t WL2 garden and low elevation pops in recent clim
wl2_garden_clim_recent <- all_clim %>% 
  filter(parent.pop=="WL2_Garden", year>1993) %>%
  rename_with(.cols = c(8:22), ~paste0(.x, '_WL2')) %>% 
  select(Season, WL2_year=year, cwd_WL2:ppt_coldest_month_WL2)
sub_dist_recent <- all_clim %>% 
  filter(parent.pop!="WL2_Garden") %>% 
  group_by(parent.pop, elevation.group, elev_m, timeframe, Season) %>% 
  summarise_at(c("cwd", "pck", "ann_tmean", "ann_ppt"),
               c(mean), na.rm = TRUE) %>% #get 30 year averages
  left_join(wl2_garden_clim_recent) %>% 
  mutate(ann_tmean_dist=ann_tmean - ann_tmean_WL2) 
sub_dist_recent %>% filter(timeframe=="recent") %>% 
  filter(Season=="Growth Season", elevation.group=="low") %>% 
  ungroup() %>% 
  summarise(meanTempDist=mean(ann_tmean_dist)) #1.883408	

sub_dist_recent %>% filter(timeframe=="historic") %>% 
  filter(Season=="Growth Season", elevation.group=="low") %>% 
  ungroup() %>% 
  summarise(meanTempDist=mean(ann_tmean_dist))
#1.181389
```

### Average subtraction distance
```{r}
sub_dist_avg <- sub_dist %>% 
  group_by(parent.pop, elevation.group, elev_m, timeframe, Season) %>% 
  summarise_at(c("cwd_dist", "pck_dist", "ann_tmean_dist", "ann_ppt_dist"),
               c(mean), na.rm = TRUE) %>% 
  rename_with(.cols = c(6:9), ~paste0(.x, '_AVG'))
```
...
#2023
sub_dist_2023 %>% 
  ggplot(aes(x=fct_reorder(parent.pop, elev_m), y=cwd_dist, group=parent.pop, fill=elev_m)) +
  geom_col(width = 0.7,position = position_dodge(0.75)) +
  scale_y_continuous(expand = c(0, 0)) +
  scale_fill_gradient(low = "#F5A540", high = "#0043F0") +
  labs(y="CWD Distance \n from WL2", fill="Elevation (m)", x="Population") +
...
sub_dist_2023 %>% 
  ggplot(aes(x=fct_reorder(parent.pop, elev_m), y=pck_dist, group=parent.pop, fill=elev_m)) +
  geom_col(width = 0.7,position = position_dodge(0.75)) +
  scale_y_continuous(expand = c(0, 0)) +
  scale_fill_gradient(low = "#F5A540", high = "#0043F0") +
  labs(y="PCK Distance \n from WL2", fill="Elevation (m)", x="Population") +
...
sub_dist_2023 %>% 
  ggplot(aes(x=fct_reorder(parent.pop, elev_m), y=ann_tmean_dist, group=parent.pop, fill=elev_m)) +
  geom_col(width = 0.7,position = position_dodge(0.75)) +
  scale_y_continuous(expand = c(0, 0)) +
  scale_fill_gradient(low = "#F5A540", high = "#0043F0") +
...
sub_dist_2023 %>% 
  ggplot(aes(x=fct_reorder(parent.pop, elev_m), y=ann_ppt_dist, group=parent.pop, fill=elev_m)) +
  geom_col(width = 0.7,position = position_dodge(0.75)) +
  scale_y_continuous(expand = c(0, 0)) +
  scale_fill_gradient(low = "#F5A540", high = "#0043F0") +
  labs(y="PPT Distance \n from WL2", fill="Elevation (m)", x="Population") +
...
#average 
sub_dist_avg %>% 
  ggplot(aes(x=fct_reorder(parent.pop, elev_m), y=cwd_dist_AVG, group=parent.pop, fill=elev_m)) +
  geom_col(width = 0.7,position = position_dodge(0.75)) +
  scale_y_continuous(expand = c(0, 0)) +
  scale_fill_gradient(low = "#F5A540", high = "#0043F0") +
  labs(y="CWD Distance \n from WL2", fill="Elevation (m)", x="Population") +
...
sub_dist_avg %>% 
  ggplot(aes(x=fct_reorder(parent.pop, elev_m), y=pck_dist_AVG, group=parent.pop, fill=elev_m)) +
  geom_col(width = 0.7,position = position_dodge(0.75)) +
  scale_y_continuous(expand = c(0, 0)) +
  scale_fill_gradient(low = "#F5A540", high = "#0043F0") +
  labs(y="PCK Distance \n from WL2", fill="Elevation (m)", x="Population") +
...
sub_dist_avg %>% 
  ggplot(aes(x=fct_reorder(parent.pop, elev_m), y=ann_tmean_dist_AVG, group=parent.pop, fill=elev_m)) +
  geom_col(width = 0.7,position = position_dodge(0.75)) +
  scale_y_continuous(expand = c(0, 0)) +
  scale_fill_gradient(low = "#F5A540", high = "#0043F0") +
  labs(y="Temp Distance \n from WL2", fill="Elevation (m)", x="Population") +
...
sub_dist_avg %>% 
  ggplot(aes(x=fct_reorder(parent.pop, elev_m), y=ann_ppt_dist_AVG, group=parent.pop, fill=elev_m)) +
  geom_col(width = 0.7,position = position_dodge(0.75)) +
  scale_y_continuous(expand = c(0, 0)) +
  scale_fill_gradient(low = "#F5A540", high = "#0043F0") +
  labs(y="PPT Distance \n from WL2", fill="Elevation (m)", x="Population") +
...
temp_dist_avg_prep <- sub_dist_avg %>% 
  select(parent.pop:Season, ann_tmean_dist_AVG) %>% 
  mutate(timeframe=if_else(timeframe=="historic", "Historic", "Recent"),
         Season=if_else(Season=="Growth Season", "GrwSsn", "Wtr_Year")) %>% 
  pivot_wider(names_from = timeframe:Season, values_from = ann_tmean_dist_AVG, names_prefix = "AvgTempDist_")

ppt_dist_avg_prep <- sub_dist_avg %>% 
  select(parent.pop:Season, ann_ppt_dist_AVG) %>% 
  mutate(timeframe=if_else(timeframe=="historic", "Historic", "Recent"),
         Season=if_else(Season=="Growth Season", "GrwSsn", "Wtr_Year")) %>% 
  pivot_wider(names_from = timeframe:Season, values_from = ann_ppt_dist_AVG, names_prefix = "AvgPPTDist_")
...
clim_geo_dist <- all_gowers_prep %>% 
  ...
  left_join(temp_dist_avg_prep) %>% 
  left_join(ppt_dist_avg_prep) %>% 
  ...
  select(parent.pop:AvgPPTDist_Recent_Wtr_Year, Geographic_Dist, Elev_Dist)
write_csv(clim_geo_dist, "../Processed.Data/All_Clim_Dist.csv")
```

## Correlation between climate, elevation, and geographic distance
```{r}
clim_geo_dist_normalized <- clim_geo_dist %>% 
  ungroup() %>% 
  select(Geographic_Dist:Elev_Dist, GD_Recent_Wtr_Year_2023:AvgPPTDist_Recent_Wtr_Year) %>% 
  scale() #center and scale the data 

cor.clim_geo_dist = cor(clim_geo_dist_normalized)
cor.sig.clim_geo_dist <- cor.mtest(clim_geo_dist_normalized, method = "pearson") #get pearson's test p-values 
cor.clim_geo_dist
cor.sig.clim_geo_dist
```
