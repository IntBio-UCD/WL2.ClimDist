...
all_clim %>% filter(parent.pop=="WL2_Garden", year==2024, Season=="Water Year") #ann_tmean = 8.952917	
all_clim %>% filter(parent.pop=="WL2_Garden", year>1993, Season=="Water Year") %>% 
  summarise(AvgTemp=mean(ann_tmean)) #avg = 8.492419	
#temp diff = 0.460498
all_clim %>% filter(parent.pop=="WL2_Garden", year>1963, Season=="Water Year") %>% 
  summarise(AvgTemp=mean(ann_tmean)) #avg = 7.934085		
#temp diff = 1.018832

#calculate average temp diff b/t WL2 garden and low elevation pops in reecent clim
...
sub_dist_recent %>% filter(timeframe=="recent") %>% 
  filter(Season=="Water Year", elevation.group=="low") %>% 
  ungroup() %>% 
  summarise(meanTempDist=mean(ann_tmean_dist)) #7.5
