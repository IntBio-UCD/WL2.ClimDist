...
total_fecund_SUB_models_CD_GD <- total_fecund_SUB_models_CD_GD %>%
  mutate(glmer = map(f, ~ glmer(as.formula(.), 
                            family = poisson,
                            data = wl2_total_fitness_scaled)))
#convergence warnings for 3_WY_Historical, 8_GS_Recent_height, and 9_GS_Historical_height models with mf included
...
total_fecund_models_CD_GD %>% mutate(tidy=map(glmer, tidy)) %>% unnest(tidy) %>%
  select(name, term:p.value) %>% 
  filter(str_detect(term, "GD") | term=="Geographic_Dist_scaled") %>%
...
total_fecund_models_CD_GD <- tribble(
  ~name,          ~f,
  "1_pop.block",      "Total_Fitness ~  height.cm + (1|pop) + (1|block)", 
  "2_WY_Recent",      "Total_Fitness ~  AvgGD_Recent_Wtr_Year_scaled + Geographic_Dist_scaled + height.cm_scaled + (1|pop) + (1|block)",
  "3_WY_Historical",  "Total_Fitness ~  AvgGD_Historic_Wtr_Year_scaled + Geographic_Dist_scaled + height.cm_scaled + (1|pop) + (1|block)",
  "4_GS_Recent",      "Total_Fitness ~  AvgGD_Recent_GrwSsn_scaled + Geographic_Dist_scaled + height.cm_scaled + (1|pop/mf) + (1|block)", 
  "5_GS_Historical",  "Total_Fitness ~  AvgGD_Historic_GrwSsn_scaled + Geographic_Dist_scaled + height.cm_scaled + (1|pop/mf) + (1|block)"
)
#run the models 
total_fecund_models_CD_GD <- total_fecund_models_CD_GD %>%
  mutate(glmer = map(f, ~ glmer(as.formula(.), 
...
wtr_yr_recent_mod <- glmer(Total_Fitness ~  AvgGD_Recent_Wtr_Year_scaled + Geographic_Dist_scaled + height.cm_scaled + (1|pop) + (1|block), 
                           family = poisson, data = wl2_total_fitness_scaled)
#get min and max GD
min(wl2_total_fitness_scaled$AvgGD_Recent_Wtr_Year_scaled) 
max(wl2_total_fitness_scaled$AvgGD_Recent_Wtr_Year_scaled)
minmaxGD = list(AvgGD_Recent_Wtr_Year_scaled = c(-1.358033, 2.153113))

# Calculate EMMs at the min and max GD
emm_results <- emmeans(wtr_yr_recent_mod, ~ AvgGD_Recent_Wtr_Year_scaled, at = minmaxGD)
# View the results
summary(emm_results)
