...
```{r}
wl2_total_fitness_scaled_2 <- wl2_total_fitness %>% 
  rename(AvgGD_Recent_Wtr_Year=`AvgGD_Recent_Water Year`,
         AvgGD_Recent_GrwSsn=`AvgGD_Recent_Growth Season`,
         AvgGD_Historic_Wtr_Year=`AvgGD_Historic_Water Year`,
         AvgGD_Historic_GrwSsn=`AvgGD_Historic_Growth Season`) %>% 
  select(block:Long, AvgGD_Historic_GrwSsn:AvgGD_Recent_Wtr_Year, 
         AvgTempDist_Historic_GrwSsn:Geographic_Dist, height.cm, Total_Fitness) %>% 
...
geo_fixed <- wl2_total_fitness_scaled_2 %>% summarise(Geographic_Dist=mean(Geographic_Dist), Geographic_Dist_scaled=mean(Geographic_Dist_scaled))
summary(wl2_total_fitness_scaled_2)

pred_df_recent_wtr_yr <- tibble(AvgGD_Recent_Wtr_Year = seq(from = 0.2, to=0.65, length.out=46)) %>% 
...
wl2_total_fitness %>% 
  group_by(pop) %>% 
  summarise(GroupMeans=mean(Total_Fitness), GroupVars=var(Total_Fitness)) %>% 
...
```{r}
total_fecund_poisson <- glmer(Total_Fitness ~ height.cm + (1|pop/mf) + (1|block), 
                              family = poisson,
...
total_fecund_SUB_models_CD_GD <- tribble(
  ~name,          ~f,
  "1_pop.block",      "Total_Fitness ~  height.cm + (1|pop/mf) + (1|block)", 
  "2_WY_Recent",      "Total_Fitness ~  AvgTempDist_Recent_Wtr_Year + AvgPPTDist_Recent_Wtr_Year + Geographic_Dist + height.cm + (1|pop/mf) + (1|block)",
  "3_WY_Historical",  "Total_Fitness ~  AvgTempDist_Historic_Wtr_Year + AvgPPTDist_Historic_Wtr_Year + Geographic_Dist + height.cm + (1|pop/mf) + (1|block)",
  "4_GS_Recent",      "Total_Fitness ~  AvgTempDist_Recent_GrwSsn + AvgPPTDist_Recent_GrwSsn + Geographic_Dist + height.cm + (1|pop/mf) + (1|block)", 
  "5_GS_Historical",  "Total_Fitness ~  AvgTempDist_Historic_GrwSsn + AvgPPTDist_Historic_GrwSsn + Geographic_Dist + height.cm + (1|pop/mf) + (1|block)"
)
...
total_fecund_SUB_models_CD_GD <- total_fecund_SUB_models_CD_GD %>%
  mutate(glmer = map(f, ~ glmer(as.formula(.), 
                            family = poisson,
                            data = wl2_total_fitness_scaled)))
#convergence warnings for growth season models 
...
#investigate growth season models:
mod_test <- glmer(Total_Fitness ~ AvgTempDist_Historic_GrwSsn + AvgPPTDist_Historic_GrwSsn + Geographic_Dist + height.cm + (1|pop/mf) + (1|block), 
                              family = poisson,
                              data=wl2_total_fitness_scaled)
#Recent: Warning: Model failed to converge with max|grad| = 0.121913 (tol = 0.002, component 1)Warning: Model is nearly unidentifiable: very large eigenvalue
 #- Rescale variables?
#Historic: Warning: Model failed to converge with max|grad| = 0.572738 (tol = 0.002, component 1)
```
...
total_fecund_models_CD_GD <- tribble(
  ...
  "2_WY_Recent",      "Total_Fitness ~  AvgGD_Recent_Wtr_Year + Geographic_Dist + (1|pop) + (1|block)",
  "3_WY_Historical",  "Total_Fitness ~  AvgGD_Historic_Wtr_Year + Geographic_Dist + (1|pop) + (1|block)",
  "4_GS_Recent",      "Total_Fitness ~  AvgGD_Recent_GrwSsn + Geographic_Dist + (1|pop) + (1|block)", 
  "5_GS_Historical",  "Total_Fitness ~  AvgGD_Historic_GrwSsn + Geographic_Dist + (1|pop) + (1|block)"
)
#run the models 
...
mod_test <- glmer(Total_Fitness ~ AvgGD_Recent_Wtr_Year + Geographic_Dist + (1|pop/mf) + (1|block), 
                              family = poisson,
                              data=wl2_total_fitness_scaled)
#convergence issues with water year models when mf included 

#show results:
total_fecund_models_CD_GD %>% mutate(tidy=map(glmer, tidy)) %>% unnest(tidy) %>%
  select(name, term:p.value) %>% 
  filter(str_detect(term, "GD") | term=="Geographic_Dist") %>%
  drop_na(p.value) #recent water year sig
...
Recent Water Year predictions
```{r}
WY_Recent_GD_mod <- glmer(Total_Fitness ~  AvgGD_Recent_Wtr_Year_scaled +
                            Geographic_Dist_scaled + (1|pop) + (1|block),
                           family="poisson", data=wl2_total_fitness_scaled_2)
WY_Recent_GD_preds <- tibble(.pred=predict(WY_Recent_GD_mod,
                                           newdata=pred_df_recent_wtr_yr,
                                           re.form= NA, type="response")) %>% 
  bind_cols(pred_df_recent_wtr_yr) %>% 
  mutate(pop=NA)
```
...
total_fecund_models_CD_GD <- tribble(
  ~name,          ~f,
  "1_pop.block",      "Total_Fitness ~  height.cm + (1|pop/mf) + (1|block)", 
  "2_WY_Recent",      "Total_Fitness ~  AvgGD_Recent_Wtr_Year + Geographic_Dist + height.cm + (1|pop/mf) + (1|block)",
  "3_WY_Historical",  "Total_Fitness ~  AvgGD_Historic_Wtr_Year + Geographic_Dist + height.cm + (1|pop/mf) + (1|block)",
  "4_GS_Recent",      "Total_Fitness ~  AvgGD_Recent_GrwSsn + Geographic_Dist + height.cm + (1|pop/mf) + (1|block)", 
  "5_GS_Historical",  "Total_Fitness ~  AvgGD_Historic_GrwSsn + Geographic_Dist + height.cm + (1|pop/mf) + (1|block)"
)
#run the models 
total_fecund_models_CD_GD <- total_fecund_models_CD_GD %>%
  mutate(glmer = map(f, ~ glmer(as.formula(.), 
                            family = poisson,
                            data = wl2_total_fitness_scaled)))
#convergence issues with water year models
...
#investigate water year models  
mod_test <- glmer(Total_Fitness ~ AvgGD_Historic_Wtr_Year + Geographic_Dist + height.cm + (1|pop/mf) + (1|block), 
                              family = poisson,
                              data=wl2_total_fitness_scaled)
#Warning: Model failed to converge with max|grad| = 0.141704 (tol = 0.002, component 1)Warning: Model is nearly unidentifiable: very large eigenvalue
# - Rescale variables?

#pairwise test between WL2 and BH (the pop with the highest total fitness):
wtr_yr_recent_mod <- glmer(Total_Fitness ~  AvgGD_Recent_Wtr_Year + Geographic_Dist + height.cm + (1|pop/mf) + (1|block), family = poisson, data = wl2_total_fitness_scaled)
#get min and max GD
min(wl2_total_fitness_scaled$AvgGD_Recent_Wtr_Year) 
max(wl2_total_fitness_scaled$AvgGD_Recent_Wtr_Year)
...
#plus the model did not converge!
```

## Try independent model with just BH and WL2
```{r}
wl2_total_fitness_scaled_ind <- wl2_total_fitness_scaled %>% 
  filter(pop=="BH" | pop=="WL2")

wtr_yr_recent_mod_ind <- glmer(Total_Fitness ~  AvgTempDist_Recent_Wtr_Year + height.cm + (1|block), 
                               family = poisson, data = wl2_total_fitness_scaled_ind)
summary(wtr_yr_recent_mod_ind)
```

## Figures
```{r}
total_fecund_summary <- wl2_total_fitness %>% 
  rename(AvgGD_Recent_Water_Yr=`AvgGD_Recent_Water Year`) %>% 
  group_by(pop, elev_m, AvgGD_Recent_Water_Yr) %>% 
  summarise(meanTotalFecund=mean(Total_Fitness, na.rm=TRUE), semEst=sem(Total_Fitness, na.rm=TRUE))
total_fecund_summary %>% arrange(AvgGD_Recent_Water_Yr) #BH had the highest total fitness 

total_fecund_summary %>% 
  ggplot(aes(x=AvgGD_Recent_Water_Yr, y=meanTotalFecund, group = pop, color=elev_m)) +
  scale_colour_gradient(low = "#F5A540", high = "#0043F0") +
  geom_point(size=6) + 
  geom_errorbar(aes(ymin=meanTotalFecund-semEst,ymax=meanTotalFecund+semEst),width=0,linewidth = 2) +
  theme_classic() + 
  ...
  #geom_line(data=WY_Recent_GD_preds, aes(x=AvgGD_Recent_Wtr_Year, y=.pred), linewidth=2, colour="black", alpha=0.5) +
  theme(text=element_text(size=30))
ggsave("../Figures/Indiv_Fit_Comps/TotalFitness_GD_Recent_WtrYr_PredLine.png",
